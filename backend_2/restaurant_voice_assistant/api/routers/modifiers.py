"""Modifiers API router.

This module provides REST endpoints for managing modifiers (add-ons, extras)
that can be linked to menu items. Includes CRUD operations and automatic
embedding generation.

Endpoints:
    - GET /api/restaurants/{restaurant_id}/modifiers: List all modifiers
    - GET /api/restaurants/{restaurant_id}/modifiers/{modifier_id}: Get single modifier
    - POST /api/restaurants/{restaurant_id}/modifiers: Create modifier
    - PUT /api/restaurants/{restaurant_id}/modifiers/{modifier_id}: Update modifier
    - DELETE /api/restaurants/{restaurant_id}/modifiers/{modifier_id}: Delete modifier

Authentication:
    All endpoints accept JWT (frontend users) or X-Vapi-Secret (admin/scripts).
    Users can only access their own restaurant's modifiers.

Modifier Management:
    - Modifiers have optional additional price (can be $0 for free options)
    - Linked to menu items via menu_item_modifiers junction table
    - Cache is automatically invalidated on changes
    - Embeddings are regenerated in background after changes

Usage:
    Create modifier:
        POST /api/restaurants/{restaurant_id}/modifiers
        Body: {"name": "Extra Cheese", "description": "...", "price": 2.50}
"""
...
@router.post(
    ...
    description="Create a new modifier. Automatically triggers background embedding generation.",
    responses={
...
@router.put(
    ...
    description="Update a modifier. All fields are optional. Automatically triggers background embedding generation.",
    responses={
...
@router.delete(
    ...
    description="Delete a modifier. Automatically triggers background embedding generation.",
    responses={
