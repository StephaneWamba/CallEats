"""Vapi resource manager for tools and assistants.

This module provides high-level operations for managing Vapi resources using YAML
configuration files. It handles the creation, updating, and management of:
    - Function tools (defined in tools.yaml)
    - Voice assistants (defined in assistant.yaml)
    - Prompts and system messages (defined in prompts.yaml)

Key Features:
    - Loads and validates YAML configuration files
    - Creates tools and assistants programmatically
    - Manages tool-to-assistant assignments
    - Handles phone number assignments
    - Provides cleanup utilities for resource management

Configuration Files:
    - config/vapi/tools.yaml: Function tool definitions
    - config/vapi/assistant.yaml: Assistant configuration
    - config/vapi/prompts.yaml: System prompts and messages

Usage:
    from restaurant_voice_assistant.infrastructure.vapi.manager import VapiResourceManager
    
    manager = VapiResourceManager(api_key="key", backend_url="https://...")
