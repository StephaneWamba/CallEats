"""Vapi tool response formatting utilities.

This module provides functions for formatting tool call responses to Vapi.
It handles structured response creation with metadata for better TTS answers.

Key Functions:
    - build_tool_result(): Simple text response
    - build_tool_result_with_items(): Response with structured metadata
    - build_no_result(): Category-appropriate no-result response
    - build_structured_items(): Convert search results to structured format
...
Usage:
    Used by infrastructure/vapi/knowledge.py when processing tool calls.
"""
from typing import Dict, List, Any, Optional


def build_tool_result(tool_call_id: str, text: str) -> Dict:
    """Build simple tool result with text only."""
    return {
        "results": [
            {
...
def build_tool_result_with_items(tool_call_id: str, text: str, items: List[Dict[str, Any]]) -> Dict:
    """Build tool result with structured metadata items."""
    return {
        "results": [
            {
                "toolCallId": tool_call_id,
