"""Vapi server webhook event handlers.

This module handles Vapi server webhook events including:
...
Webhook Reliability:
    ...
    This module implements a fallback mechanism:
        - Schedules API fetch 30 seconds after call detection
        - Ensures complete call data is captured even if webhooks fail
        - Prevents duplicate fetches using thread-safe tracking

Key Functions:
    - handle_assistant_request(): Process assistant-request events
    - handle_status_update(): Process status-update events
    - handle_end_of_call_report(): Process end-of-call-report events

Usage:
    These functions are called from the unified server webhook endpoint
    in api/routers/vapi.py when processing Vapi events.
"""
...
from restaurant_voice_assistant.infrastructure.cache.manager import store_call_phone, get_call_phone
from restaurant_voice_assistant.domain.calls.fetch import fetch_and_store_call_from_vapi
import logging
import threading
import time
