"""Call parsing utilities for Vapi data.

This module provides utilities for parsing and storing Vapi call data including:
    - Phone number normalization
    - Vapi API response parsing
    - Call record creation with restaurant mapping

Key Features:
    - Handles Vapi field variations (startedAt/createdAt, endedAt/updatedAt)
    - Message filtering (user/assistant only)
    - Restaurant ID resolution via phone number mapping
    - Timestamp parsing and duration calculation

Usage:
    Used internally by domain/calls/fetch.py and infrastructure/vapi/server.py
    to parse and store call data from Vapi webhooks and API responses.
"""
from typing import Optional, Dict, Any
from datetime import datetime
from restaurant_voice_assistant.domain.phones.mapping import get_restaurant_id_from_phone
from restaurant_voice_assistant.domain.calls.service import create_call
import logging
