"""Call history domain service.

This module provides business logic for managing voice call records including:
...
Key Features:
    - Restaurant-scoped call history
    - Message filtering (excludes system/tool messages)
    - Call metadata storage (duration, cost, outcome)
    - Multi-tenant isolation

Message Filtering:
    Only includes user and assistant messages, filtering out system messages,
    tool calls, and other non-conversational content for cleaner transcripts.
...
Usage:
    ...
    calls = list_calls(restaurant_id="...", limit=50)
    call = get_call(call_id="...", restaurant_id="...")
"""
...
def list_calls(restaurant_id: str, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of call records ordered by started_at (descending)
    """
