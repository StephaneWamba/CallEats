"""Phone number assignment domain service.

This module orchestrates phone number assignment to restaurants. It handles
the complete flow from checking for existing phones to creating new Twilio
numbers through Vapi.

Key Features:
    - Automatic phone number assignment
    - Existing phone reuse
    - Twilio phone provisioning
    - Vapi assistant assignment

Assignment Strategy:
    1. Check for existing unassigned phone numbers
    2. If none available and Twilio configured, create new Twilio number
    3. Assign phone to shared assistant
    4. Create restaurant mapping
...
from restaurant_voice_assistant.domain.phones.twilio import create_and_assign_twilio_phone
