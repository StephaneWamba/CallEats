...
This module provides a comprehensive client for interacting with the Vapi.ai API.
It handles all Vapi resource management including tools, assistants, phone numbers,
credentials, and calls.

Key Features:
    - Full CRUD operations for Vapi resources
    - Automatic error handling and logging
    - Request timeout handling (30 seconds)
    - Voice configuration filtering (removes voice settings from requests)

Resources Managed:
    - Tools: Function tools for knowledge base queries
    - Assistants: Voice assistant configurations
    - Phone Numbers: Phone number provisioning and assignment
    - Credentials: Provider credentials (Twilio, etc.)
    - Calls: Call data retrieval

Error Handling:
    - Raises VapiAPIError for API failures
...
Usage:
    ...
    client = VapiClient(api_key="your_api_key")
    tools = client.list_tools()
    assistant = client.create_assistant(assistant_config)
"""
import requests
import logging
